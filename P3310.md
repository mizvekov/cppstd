...
introducing a new template-head which is used in place of the original template's template-head.
This associated template-head is identical to the one in the original template, except that
...
template's template-head with an equivalent template-head are equivalent to each other,
and those which replace it with a template-head which is equivalent to their original template
are equivalent to the original template.
:::

Add a new paragraph after [temp.func.order]{.sref}/5:

[5]{.pnum} [Note 3: Since, in a call context, such type deduction considers only parameters
for which there are explicit call arguments, some parameters are ignored (namely, function
parameter packs, parameters with default arguments, and ellipsis parameters).

::: add
[a]{.pnum} When deducing template template parameters as described in [temp.deduct.type],
if P is a specialization of a template template parameter, the specialized parameter will
be deduced as a variant template ([temp.pre]), except that elements of the template argument
list in A are used as the default arguments for the corresponding variant template's template
parameters, except for the initial sequence of parameters which has a matching argument in P.
