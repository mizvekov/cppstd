...
::: add
[8.2]{.pnum} - When P is a template template parameter specialization, and A is either also the same kind, or a class template specialization, the template template parameter in P will be deduced as a modified template A, as if the template argument list in A were used as the default arguments for
