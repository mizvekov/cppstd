...
Add a new paragraph after [temp.spec.partial.order]{.sref}/1:
...
::: add
[2]{.pnum} The set of template specializations for which there is no strict-pack-match ([temp.deduct.type]) is considered first, including the primary template. If there is no viable template in this first set, partial ordering considers the rest.
