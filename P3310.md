...
::: add
[5]{.pnum} Partial ordering considers first the set of templates for which there is no strict-pack-match ([temp.deduct.type]/9.1), possibly including the primary template. If there is no viable template in this first set, partial ordering considers the rest.
...
[6]{.sref} [Note 3: Since, in a call context, such type deduction considers only parameters for which there are explicit call arguments, some parameters are ignored (namely, function parameter packs, parameters with default arguments, and ellipsis parameters).
...
Reference to [temp.arg.template]/4, unchanged:
...
[During partial ordering, if A~i~ was originally a pack expansion]{.rm} [When deducing the argument list of the specializations of the invented class template X ([temp.arg.template]/4), if A~i~ is not a pack expansion, this is a _strict-pack-match_. During partial ordering, if A~i~ was originally a pack expansion]{.add}:
