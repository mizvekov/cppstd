...
::: add
[3]{.pnum} When deducing the argument list of X as specified in [temp.arg.template]/4, partial ordering considers first the set of templates for which [temp.deduct.type]/9.1 does not apply, possibly including the primary template. If there is no viable template in this first set, partial ordering considers the rest.
:::
...
::: add
[9.1]{.pnum} [Except when simultaneously partial ordering and deducing the argument list of X as specified in [temp.arg.template]/4, ]{.add} if P~i~ is a pack expansion, then the pattern of P~i~ is compared with each remaining argument in the template argument list of A. Each comparison deduces template arguments for subsequent positions in the template parameter packs expanded by P~i~. During partial ordering, if A~i~ was originally a pack expansion:
:::
...
::: add
[2.4]{.pnum} When deducing the argument list of X as specified in [temp.arg.template]/4, [temp.deduct.type]/9.1 does not apply to F1, and does to F2.
