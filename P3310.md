---
title: Solving issues introduced by P0522R0
document: P3310R4
date: 2024-10-15
audience:
...
* Restricted effects of the change so it only applies to partial ordering.
...
Here problem goes outside of the scope of partial ordering.
...
Lastly, we propose to introduce a new kind of conversion for the purpose of
solving the partial ordering issue.
This conversion would be defined by the presence of a pack on the argument side matching
...
When matching candidates against the primary template and partial specialziations,
...
::: add
[2.4]{.pnum} Except while partial ordering, when deducing the argument list of X as specified in 13.4.4 [temp.arg.template]/4, F1 does not apply [temp.deduct.type]{.sref}/9.2, and F2 does.
