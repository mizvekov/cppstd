...
Modify starting from [temp.arg.template]{.sref}/3:
...
When instantiating A\<B\>, matching B to #1 is not a _strict-pack-match_ ([temp.deduct.type]), while #1 is, so #1 is a better match.
:::
