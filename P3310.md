...
Add new paragraphs after [temp.func.order]{.sref}/4:

[4]{.pnum} Using the transformed function template's function type, perform type deduction against the other template as described in [temp.deduct.partial].

::: add
[5]{.pnum} Partial ordering considers first the set of template specializations for which there is no strict-pack-match ([temp.deduct.type]), including the primary template. If there is no viable template in this first set, partial ordering considers the rest.
...
Modify [temp.arg.template]{.sref}/4:

[4]{.pnum} A template template-parameter P is at least as specialized as a template template-argument A if, given the following rewrite to two function templates, the function template corresponding to P is at least as specialized as the function template corresponding to A according to the partial ordering rules for function templates. [The _ttp-invented-class-template_ is]{.add} [Given]{.rm} an invented class template [X]{.rm} with the template-head of A (including default arguments and requires-clause, if any)[. Given a _ttp-invented-class-template_ "X"]{.add}:

Modify [temp.deduct.type]{.sref}/9, adding a bulleted list:

[9]{.pnum} If P has a form that contains \<T\>[ or]{.rm}[,]{.add} \<i\>, [or \<TT\>, ]{.add} then each argument P~i~ of the respective template argument list of P is compared with the corresponding argument A~i~ of the corresponding template argument list of A. If the template argument list of P contains a pack expansion that is not the last template argument, the entire template argument list is a non-deduced context.
...
[During partial ordering, if A~i~ was originally a pack expansion]{.rm} [A _strict-pack-match_ occurs when deducing the argument list of the specializations of a _ttp-invented-class-template_ ([temp.arg.template]), and A~i~ is not a pack expansion. During partial ordering, if A~i~ was originally a pack expansion]{.add}:
...
template <template <class> class TT> struct C<TT>; // OK, more specialized than #1

template <template <class> class TT> struct D;
template <template <class...> class TT> struct D<TT>; // error: not more specialized than #1
```
- end example\]
...
::: add
[2.4]{.pnum} F1 does not have a strict-pack-match ([temp.deduct.type]), and F2 does.
