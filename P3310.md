...
::: add
[5]{.pnum} Partial ordering considers first the set of template specializations for which there is no strict-pack-match ([temp.deduct.type]/9.1), including the primary template. If there is no viable template in this first set, partial ordering considers the rest.
...
[During partial ordering, if A~i~ was originally a pack expansion]{.rm} [A _strict-pack-match_ occurs when deducing the argument list of the specializations of a _ttp-invented-class-template_, and A~i~ is not a pack expansion. During partial ordering, if A~i~ was originally a pack expansion]{.add}:
